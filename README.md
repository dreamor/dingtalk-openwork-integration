...
- **拥有开发者权限**：选择您有开发者权限的组织，或先获取开发者权限

### 步骤一：登录钉钉开放平台

访问钉钉开发者后台：[https://open-dev.dingtalk.com](https://open-dev.dingtalk.com)，使用钉钉账号扫码登录。
...
| 字段 | 说明 | 是否必填 |
|------|------|----------|
| 应用名称 | 填写自定义名称（如：OpenCode AI 助手） | 是 |
| 应用描述 | 描述应用用途 | 是 |
| 应用Logo | 上传应用图标 | 否（可使用默认图标） |
...
创建成功后，在「凭证与基础信息」或「开发信息」页面，找到以下关键信息：
...
> ⚠️ **重要提示**：
> - AppKey 和 AppSecret 是应用的关键信息，也是操作应用数据的核心参数
> - 请妥善保管，切勿轻易提供给他人使用
