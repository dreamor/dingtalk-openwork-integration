...
// Stream 服务引用（在启动时设置）
let streamService: {
  sendTextMessage: (conversationId: string, content: string, mentionList?: string[]) => Promise<void>;
  sendMarkdownMessage: (conversationId: string, title: string, text: string) => Promise<void>;
} | null = null;
